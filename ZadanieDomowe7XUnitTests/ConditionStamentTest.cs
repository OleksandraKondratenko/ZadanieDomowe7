...
namespace ZadanieDomowe7XUnitTests
{
    public class ConditionStamentTest
    {
        ...
        [InlineData(8, 8, 64)]
        [InlineData(5, 8, -3)]
        public void CompareTwoNumbers_WhenValidInputPassed_ShouldCalculateCondition(int num1, int num2, int expected)
        {
        ...
        [InlineData(-5, -6, 3)]
        [InlineData(5, -6, 4)]
        public void GetQuarterOnTheAxisOfReferenceIsPassed_WhenValidInputPassed_ShouldFindPlacePoint(int x, int y, int expected)
        {
            int result = ConditionStatement.GetQuarterOnTheAxisOfReference(x, y);

            Assert.Equal(expected, result);
        }

        [Theory]
        [InlineData(5, 6, 1, 1, 5, 6)]
        [InlineData(-5, 6, 2, -5, 2, 6)]
        public void SortThreeNumberByAscending_WhenTestIsPassed_ShouldBeCalculate(int num1, int num2, int num3, int expectedNum1, int expectedNum2, int expectedNum3)
        {
